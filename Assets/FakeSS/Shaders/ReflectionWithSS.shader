...
half4 frag (unity_v2f_deferred i) : SV_Target
{
	...
	float specularColor = data.specularColor;
	if (abs(gbuffer2.a - 0.5f) < 0.2f)
	{
		specularColor = half3(0, 0, 0);
	}

    Unity_GlossyEnvironmentData g = UnityGlossyEnvironmentSetup(data.smoothness, d.worldViewDir, data.normalWorld, specularColor);
    ...
    half3 rgb = UNITY_BRDF_PBS (0, specularColor, oneMinusReflectivity, data.smoothness, data.normalWorld, -eyeVec, light, ind).rgb;
